...
# Assign secrets and Tokens to Fetch the data from Twitter API
...
for tweet in tweepy.Cursor(api.search_tweets, q=new_search, count=1000,
                           lang="en",
                           since_id=0).items():
    csvWriter.writerow([tweet.id, tweet.created_at, tweet.text.encode('utf-8'), tweet.user.screen_name.encode('utf-8'),
                        tweet.user.verified, tweet.user.followers_count, tweet.favorite_count, tweet.retweet_count,
